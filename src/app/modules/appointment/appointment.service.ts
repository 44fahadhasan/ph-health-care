...
const bookAppointment = async (
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${envVars.FRONT_END_URL}/dashboard/payment/payment-success`,

      // cancel_url: `${envVars.FRONT_END_URL}/dashboard/payment/payment-failed`,
      cancel_url: `${envVars.FRONT_END_URL}/dashboard/appointments`,
    });
...
const initiatePayment = async (appointmentId: string, user: IReqUser) => {
  const patientData = await prisma.patient.findUniqueOrThrow({
    where: {
      email: user.email,
    },
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: `${envVars.FRONT_END_URL}/dashboard/payment/payment-success?appointment_id=${appointmentData.id}&payment_id=${appointmentData.payment.id}`,

    cancel_url: `${envVars.FRONT_END_URL}/dashboard/appointments?error=payment_cancelled`,
  });
